...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //"액션을 드래그 대신 코드로 연결해보자"
        //addTarget는 옛날 objective-C 코드가 남아 있음 //함수 호출 연산자인 "괄호"는 생략하고 "함수 이름"만(미리 실행되는걸 방지하기 위해 "연결"만 해준다는 의미로 함수 이름만 작성) //함수에 @objc도 붙여줘야
    ...
    @objc func presentTestButtonClicked() {
        ...
        //"시점"의 문제: 이 코드가 실행될 시점에 아웃렛이 다 만들어져 있어야 문제가 없음 //근데, 아웃렛은 생각보다 늦게 만들어져서 에러 발생.(아웃렛이 있어야 글자를 넣지 그래서 없다는 nil 에러)
        //그래서 화면 전환을 할 때는 아웃렛을 가져오지 않도록 개발을 하자 : 우회적인 방법으로 해
        //그럼 다음화면에 값을 전달하고 싶을 땐?
        
        
        //화면 전환하면서 데이터를 전달하기 위한 구성2. contents라는 다음 화면의 공간에 내용을 넣어주기
